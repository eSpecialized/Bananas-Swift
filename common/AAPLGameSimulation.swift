...
class AAPLGameSimulation: SCNScene, SCNSceneRendererDelegate, SCNPhysicsContactDelegate {
    ...
    fileprivate var _gameState: AAPLGameState = .paused
    fileprivate var controller: GCController? {
        didSet {
    ...
    fileprivate var _walkSpeed: CGFloat = 0
    fileprivate var _previousUpdateTime: TimeInterval = 0
    fileprivate var _previousPhysicsUpdateTime: TimeInterval = 0
    fileprivate var _deltaTime: TimeInterval = 0
    
    fileprivate var desaturationTechnique: SCNTechnique!
    
    // Singleton for easy lookup
    static let sim = AAPLGameSimulation()
    
    fileprivate func setupTechniques() {
        ...
        self.desaturationTechnique = SCNTechnique(dictionary: NSDictionary(contentsOf: url!)! as! [String : AnyObject])
    ...
    fileprivate func setPostGameFilters() {
    ...
    fileprivate func setPauseFilters() {
        SCNTransaction.begin()
    ...
    fileprivate func setPregameFilters() {
    ...
    fileprivate func setIngameFilters() {
        SCNTransaction.begin()
    ...
    */
    ...
    fileprivate func playerCollideWithContact(_ node: SCNNode, point contactPoint: SCNVector3) {
    ...
    fileprivate func handleCollideForCoconut(_ coconut: AAPLCoconut) {
    ...
    fileprivate func didSetController(_: GCController?) {
    ...
    func playMusic(_ soundFileName: String?) {
        guard let soundFileName = soundFileName, self.gameUIScene?.action(forKey: soundFileName) != nil else {
                return
        }
...
//                SCNSceneSource.LoadingOption.convertToYUp.rawValue: true as AnyObject,
