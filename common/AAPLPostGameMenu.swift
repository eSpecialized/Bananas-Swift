...
class AAPLPostGameMenu: SKNode {
    ...
    fileprivate var myLabel: SKLabelNode
    fileprivate var bananaText: SKLabelNode!
    fileprivate var bananaScore: SKLabelNode!
    fileprivate var coinText: SKLabelNode!
    fileprivate var coinScore: SKLabelNode!
    fileprivate var totalText: SKLabelNode!
    fileprivate var totalScore: SKLabelNode!
    ...
    init(size frameSize: CGSize, andDelegate gameStateDelegate: AAPLGameUIState?) {
        ...
        self.run(flyup, completion: {
            //-- fly up the bananas collected.
            self.bananaText.run(flyupBananas)
            self.bananaScore.run(flyupBananasScore, completion: {
                //-- count!
                self.bananaScore.run(countUpBananas, completion: {
                    self.bananaScore.text = String(bananasCollected)
                    self.coinText.run(flyupCoins)
                    self.coinScore.run(flyupCoinsScore, completion: {
                        //-- count
                        self.coinScore.run(countUpCoins, completion: {
                            self.coinScore.text = String(coinsCollected)
                            self.totalText.run(flyupTotal)
                            self.totalScore.run(flyupTotalScore, completion: {
                                self.totalScore.run(countUpTotal, completion: {
